...
from agent_code.Q_learner.helpers import WALL_RIGHT, WALL_UP, WALL_DOWN, WALL_LEFT, ACTIONS, wall_action_index
...
def setup(self):
    ...
    self.new_state = None
    self.old_state = None
    self.valid_state_list = ['WALL_UP', 'WALL_DOWN', 'WALL_LEFT', 'WALL_RIGHT', 'NO_OBSTACLE']
    if self.train:
...
def check_the_presence_of_obstacles(self, game_state: dict) -> np.array:
    ...
    if arena[coord_x + 1][coord_y] == 1 or (coord_x + 1, coord_y) == -1:
        ...
        obstacle.append(wall_action_index['WALL_RIGHT'])
    elif arena[coord_x - 1][coord_y] == 1 or (coord_x - 1, coord_y) == -1:
        # obstacle.append({'WALL_LEFT': WALL_LEFT})
        obstacle.append(wall_action_index['WALL_LEFT'])
    elif arena[coord_x][coord_y + 1] == 1 or (coord_x, coord_y + 1) == -1:
        # obstacle.append({'WALL_UP': WALL_UP})
        obstacle.append(wall_action_index['WALL_UP'])
    elif arena[coord_x][coord_y - 1] == 1 or (coord_x, coord_y - 1) == -1:
        # obstacle.append({'WALL_DOWN': WALL_DOWN})
        obstacle.append(wall_action_index['WALL_DOWN'])
    else:
        obstacle.append(wall_action_index['NO_OBSTACLE'])
